...
import static com.nxp.sampletaplinx.Constants.DEFAULT_SECTOR_CLASSIC;
import static com.nxp.sampletaplinx.Constants.KEY_AES128_DEFAULT;
import static com.nxp.sampletaplinx.Constants.TAG;
import static com.nxp.sampletaplinx.Constants.UNABLE_TO_READ;
...
//                    ntag424DNA.changeKey(0, aesKey, null, (byte) 0x00);
            ...
            try {
                ...
                sdmSettings.setSdmAccessRights(new byte[]{(byte) 0x0E, 0x00, 0x00});
                sdmSettings.setUidOffset(new byte[]{0x36, 0x00, 0x00});
                sdmSettings.setSdmReadCounterOffset(new byte[]{0x51, 0x00, 0x00});
                sdmSettings.setSdmMacOffset(new byte[]{0x67, 0x00, 0x00});
                sdmSettings.setSdmMacInputOffset(new byte[]{0x31, 0x00, 0x00});
 ...
 */
    ...
    String mifareDUOXCardLogic(MainActivity activity, IMifareDUOX mifareDUOX) {
        mifareDUOX.getReader().connect();
        mifareDUOX.getReader().setTimeout(2000);

        stringBuilder.delete(0, stringBuilder.length());
        stringBuilder.append(activity.getString(R.string.Card_Detected)).append(
                mifareDUOX.getType().getTagName()).append(activity.getString(R.string.LINE_BREAK));

        stringBuilder.append(activity.getString(R.string.UID)).append(
                Utilities.dumpBytes(mifareDUOX.getUID())).append(
                activity.getString(R.string.LINE_BREAK));

        try {
            stringBuilder.append(activity.getString(R.string.SIZE)).append(
                    mifareDUOX.getTotalMemory()).append(activity.getString(R.string.LINE_BREAK));

            stringBuilder.append(activity.getString(R.string.Availabe_Size)).append(
                    mifareDUOX.getFreeMemory()).append(activity.getString(R.string.LINE_BREAK));

            byte[] getVersion = mifareDUOX.getVersion();
            if (getVersion[0] == (byte) 0x04) {
                stringBuilder.append(activity.getString(R.string.Vendor_ID)).append(
                        activity.getString(R.string.NXP)).append(
                        activity.getString(R.string.LINE_BREAK));
            ...
            if (getVersion[6] == (byte) 0x05) {
                stringBuilder.append(activity.getString(R.string.PROTOCOL)).append(
                        activity.getString(
                                R.string.PROTOCOL_VALUE_DefireEV2));
            } else if (getVersion[6] == (byte) 0x20) {
                stringBuilder.append(activity.getString(R.string.PROTOCOL)).append(
                        activity.getString(
                                R.string.PROTOCOL_I2C));
            } else if (getVersion[6] == (byte) 0x25) {
                stringBuilder.append(activity.getString(R.string.PROTOCOL)).append(
                        activity.getString(
                                R.string.PROTOCOL_I2C) + " and " + activity.getString(
                                R.string.PROTOCOL_VALUE_DefireEV2));
            } else {
                stringBuilder.append(activity.getString(R.string.PROTOCOL)).append(
                        activity.getString(R.string.PROTOCOL_UNKNOWN));
